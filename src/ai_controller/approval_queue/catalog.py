...
ACTION_CATALOG: Tuple[ActionSpec, ...] = (
    ...
    ActionSpec(
        ...
        description="Cut an endpoint off the network to stop lateral movement.",
        category="edr",
        risk="critical",
        execution="ready_if_configured",
        integration="edr",
        gated_mcp_tool="isolate_endpoint",
        notes="Requires an EDR client (Elastic Defend). Payload is stored until EDR is connected.",
        fields=_fields(
    ...
    ActionSpec(
        ...
        description="Restore network connectivity for an isolated endpoint.",
        category="edr",
        risk="high",
        execution="ready_if_configured",
        integration="edr",
        gated_mcp_tool="release_endpoint_isolation",
        notes="Requires EDR. Inverse of isolate_endpoint.",
        fields=_fields(
            FieldSpec("endpoint_id", "Endpoint ID", True),
            FieldSpec("hostname", "Hostname", False),
            FieldSpec("reason", "Reason", False),
    ...
    ActionSpec(
        ...
        description="Propose a detection-rule or exception change after a noisy or FP alert.",
        category="detections",
        risk="low",
        execution="ready_if_configured",
        integration="eng",
        gated_mcp_tool="create_fine_tuning_recommendation",
        notes="Posts to the engineering fine-tune board when Trello/ClickUp/GitHub is configured; otherwise stored here.",
        fields=_fields(
    ...
    ActionSpec(
        ...
        description="Request logging, telemetry, or coverage that the investigation was missing.",
        category="detections",
        risk="low",
        execution="ready_if_configured",
        integration="eng",
        gated_mcp_tool="create_visibility_recommendation",
        notes="Posts to the visibility/engineering board when configured.",
        fields=_fields(
            FieldSpec("title", "Title", True),
            FieldSpec("description", "Description", True),
